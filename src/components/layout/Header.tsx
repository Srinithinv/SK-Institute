  ...
  return (
    <>
      ...
      <header className={`fixed top-0 left-0 right-0 z-[100] transition-all duration-300 ${isScrolled ? 'shadow-lg' : ''}`}>
        ...
        <div className={`py-4 transition-colors duration-300 ${isScrolled ? 'bg-brand' : 'bg-transparent'}`}>
        ...
        <nav className="hidden md:flex items-center gap-8">
          {NAV_LINKS.map((link) => (
            <Link
              ...
              className="text-sm font-medium text-white/80 hover:text-white transition-colors drop-shadow-md"
            >
              {link.label}
            </Link>
          ))}
          <button
            onClick={() => openBooking()}
            className="px-5 py-2.5 rounded-lg bg-white text-brand text-sm font-semibold hover:bg-gray-100 transition-colors focus-visible:ring-2 focus-visible:ring-brand focus-visible:ring-offset-2 focus-visible:ring-offset-dark shadow-lg"
          >
        ...
        {/* Mobile Menu Toggle */}
        <button
          className="md:hidden p-2 -mr-2 text-white hover:text-gray-200 focus-visible:ring-2 focus-visible:ring-white rounded-md drop-shadow-md"
