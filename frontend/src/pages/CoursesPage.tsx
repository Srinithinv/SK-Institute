...
import { Globe, Star, Clock, Users, ChevronLeft, ChevronRight } from 'lucide-react';

export function CoursesPage() {
  const categories = [
    "Artificial Intelligence", "Data Science & Analytics", "Machine Learning", 
    "Full Stack Development", "Web Development", "Java Technologies", 
    "Python Technologies", "Cloud Computing", "DevOps", "Cybersecurity", 
    "Software Testing", "Database Technologies", "Mobile App Development", 
    "UI/UX & Design", "Digital Marketing", "Business & Management", 
    "Finance & Accounting", "SAP & ERP", "IoT & Embedded Systems"
  ];
  ...
  return (
    <div className="min-h-screen bg-white font-sans relative">
      ...
      <div className="relative z-10 pt-24 pb-20 max-w-[1280px] mx-auto px-6 md:px-8">
        ...
        <div className="mb-10 w-full rounded-xl border border-slate-100 shadow-sm bg-[#faf8fc] p-2">
          <div className="flex flex-wrap items-center justify-center gap-2 w-full">
            {categories.map((cat) => (
              <button
                ...
                className={`relative px-6 py-3 whitespace-nowrap text-[14px] transition-colors duration-200 rounded-lg ${
                  activeCategory === cat 
                    ? 'bg-[#6d28d9] text-white font-medium shadow-md' 
                    : 'bg-white text-slate-600 font-normal hover:bg-slate-50 border border-slate-200'
                }`}
              >
                {cat}
              </button>
            ...
            <AnimatePresence mode="wait">
              {displayedCourses.map((course) => (
                <motion.div
                  ...
                  key={course.id}
                  className="flex-none w-[280px] md:w-[290px] snap-start bg-white border border-slate-200 rounded-2xl cursor-pointer hover:shadow-lg transition-shadow duration-300 flex flex-col overflow-hidden"
                >
        ...
        <div className="flex flex-col md:flex-row items-center justify-between relative mt-8 gap-8 pb-4">
          <div className="flex items-center gap-4 mx-auto absolute left-1/2 transform -translate-x-1/2">
            <button 
